...
class LaikagoEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.counter_1 += distance_traveled
        ...
        if self.counter == 4800 or pos[2] < 0.25:  # Adjust threshold if needed:
            reward = self.counter_1 + self.counter_0
            self.counter_0 = 0
            self.counter_1 = 0
            terminated = True
            return obs, reward, terminated, truncated, {}
        else:
            return obs, self.counter_0, terminated, truncated, {}
...
# --------------------- Testing/Visualization Phase ------------------------
    with open("Results2.txt", "w") as file:
        ...
        robot_name = f"ppo_laikago_TORQUE_CONTROL_1"
        if episode_rewards:
