...
class LaikagoEnv(gym.Env):
    def __init__(self,
        ...
        # linear velocity (3), angular velocity (3), joint positions (8) , joint velocities (8), applied torques (8), foot on the floor= 41 dimensions.
        ...
        self.healthy_reward = 1.0  # +1 per step if not fallen
        self.forward_weight = 1.0  # scales forward reward
        self.ctrl_cost_weight = 1e-3  # scales torque penalty
        self.contact_cost_weight = 1e-3  # scales contact penalty
        self.frame_skip = 1  # if you step simulation multiple times per action
        self.dt = 1 / 240.0  # PyBullet default timestep
        self.prev_y = 0.0
    ...
    def step(self, action):
        ...
        # 1) Observation & current base position
        obs = self._get_observation()
        pos, _ = p.getBasePositionAndOrientation(self.robot)
        y = pos[1]

        # 2) healthy_reward (alive bonus)
        is_alive = (pos[2] > 0.25)  # e.g. base height > 0.25
        healthy_r = float(self.healthy_reward) if is_alive else 0.0

        # 3) forward_reward = v_forward * dt * weight
        delta_y = y - self.prev_y
        v_forward = delta_y / (self.dt * self.frame_skip)
        forward_r = self.forward_weight * v_forward

        # 4) ctrl_cost = -ctrl_cost_weight * sum(action^2)
        ctrl_c = self.ctrl_cost_weight * np.sum(np.square(action))

        # 5) contact_cost = -contact_weight * sum(|f_ext|)
        #    `external_forces` is an array of shape (n_contacts, 6): (fx,fy,fz, tx,ty,tz)

        contact_c = self.contact_cost_weight * np.sum(np.abs(self.latest_ext_forces))

        # total reward
        reward = healthy_r + forward_r - ctrl_c - contact_c
    ...
    def _get_observation(self):
        ...
        ext_forces = []
        for pt in contacts:
            # Some contact points may not have force data fully filled
            force = pt[9] if isinstance(pt[9], (list, tuple)) and len(pt[9]) == 3 else [0.0, 0.0, 0.0]
            torque = pt[10] if isinstance(pt[10], (list, tuple)) and len(pt[10]) == 3 else [0.0, 0.0, 0.0]
            ext_forces.append(force + torque)
        ext_forces = np.array(ext_forces, dtype=np.float32)
        self.latest_ext_forces = ext_forces
...
def velocity():
    ...
    if not os.path.exists(f"ppo_laikago_TORQUE_CONTROL_3_OBS.zip"):
        ...
        env = [make_laikago_env(path, render=False, seed=seed, env_id=i) for i in range(8)]
        ...
        custom_arch = dict(pi=[256, 256], vf=[256, 256])
        ...
        env.save(f"ENV_normalized_ppo_laikago_TORQUE_CONTROL_3_OBS.pkl")
        print(f" DOG Trained")
        env.close()
# ::::::::::::::: Train the PPO model on this environment ::::::::::::::::::::::
...
# --------------------- Testing/Visualization Phase ------------------------
    with open("Results2.txt", "w") as file:
        eval_env = [make_laikago_env(path, render=True, seed=seed, env_id=1)]
        eval_env = SubprocVecEnv(eval_env)
        eval_env = VecNormalize.load("ENV_normalized_ppo_laikago_TORQUE_CONTROL_3_OBS.pkl", eval_env)
