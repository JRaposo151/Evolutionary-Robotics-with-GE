...
def velocity():
    ...
    if not os.path.exists(f"ppo_laikago_TORQUE_CONTROL_2.zip"):
        # Train the PPO model on this environment
        ...
        model = PPO(policy='MlpPolicy',
                            ...
                            gae_lambda=0.98,
                            ent_coef=1e-4,  # Increase Entropy Regularization This term encourages exploration by penalizing certainty (i.e., encourages the policy to remain more "random").
                            verbose=1,
                            seed=seed,
        ...
        model.save(f"ppo_laikago_TORQUE_CONTROL_2")
        print(f" DOG Trained")
        env.close()
# ::::::::::::::: Train the PPO model on this environment ::::::::::::::::::::::
...
# --------------------- Testing/Visualization Phase ------------------------
    with open("Results2.txt", "w") as file:
        ...
        model = PPO.load(f"ppo_laikago_TORQUE_CONTROL_2")
