...
class LaikagoEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for i, joint in enumerate(self.movable_joints):
            p.setJointMotorControl2(self.robot, joint, p.TORQUE_CONTROL, force=action[i]*75)
        p.stepSimulation()

        obs, foot = self._get_observation()
    ...
    def _get_observation(self):
        ...
        # Batch joint states
        js = p.getJointStates(self.robot, self.movable_joints)
        joint_states_position = np.array([s[0] for s in js], dtype=np.float32)
        ...
        obs = np.hstack([np.array(pos), np.array(ori), np.array(lin_vel), np.array(ang_vel), joint_states_position, joint_states_velocity, foot_contact_vector])
        return  obs.astype(np.float32), foot_contact_vector
...
def velocity():
    ...
    if not os.path.exists(f"ppo_laikago_TORQUE_CONTROL_1.zip"):
        # Train the PPO model on this environment
        ...
        model = PPO(policy='MlpPolicy',
                            ...
                            gae_lambda=0.95,
                            ent_coef= 1e-2,  # Increase Entropy Regularization This term encourages exploration by penalizing certainty (i.e., encourages the policy to remain more "random").
                            target_kl=0.01,
                            verbose=1,
                            seed=seed,
                            device='cuda:0',
        ...
        model.save(f"ppo_laikago_TORQUE_CONTROL_1")
        env.save(f"ENV_normalized_ppo_laikago_TORQUE_CONTROL_1.pkl")
        print(f" DOG Trained")
        env.close()
# ::::::::::::::: Train the PPO model on this environment ::::::::::::::::::::::
...
# --------------------- Testing/Visualization Phase ------------------------
    with open("Results2.txt", "w") as file:
        eval_env = [make_laikago_env(path, render=True, seed=seed, env_id=3) for i in range(1)]
        eval_env = SubprocVecEnv(eval_env)
        eval_env = VecNormalize.load("ENV_normalized_ppo_laikago_TORQUE_CONTROL_1.pkl", eval_env)
        #  do not update them at test time
        eval_env.training = False
        # reward normalization is not needed at test time
        eval_env.norm_reward = False
        model = PPO.load(f"ppo_laikago_TORQUE_CONTROL_1")
